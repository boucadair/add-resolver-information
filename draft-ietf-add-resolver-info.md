...
#  Resolver Information Keys/Values {#key-val}
   ...
   : It contains the signature of the RESINFO RR excluding the "sig"	
     ...
     resolver using all of the attributes in the RESINFO RR except the "sig"	attribute. The signature algorithm MUST be compatible with the
public key in the DNS resolver's end-entity certificate.  As a	reminder, the server's end-entity certificate's public key will be	
compatible with the selected authentication algorithm from the	client's "signature_algorithms" TLS extension ({{Section 4.4.2.2 of !RFC8446}}).
